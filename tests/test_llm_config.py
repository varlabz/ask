...
class TestCreateModelFromLLMConfig:
    ...
    def test_google_gemini_model(self, monkeypatch):
        ...
        monkeypatch.setattr("pydantic_ai.providers.google_gla.GoogleGLAProvider", DummyGoogleGLAProvider)
    ...
    def test_invalid_google_model_format(self):
        ...
        with pytest.raises(ValueError, match="Invalid model format: google-gemini-2.5-pro"):
            create_model(llm_config)

    def test_invalid_google_provider(self):
        llm_config = LLMConfig(
            model="googlex:gemini-2.5-pro",
            api_key="google-key",
            base_url=None
        )
