...
def find_next_available_port(
    start: int = 8000, end: int = 9999, host: str = "localhost"
) -> int | None:
    ...
    if messages:
        for msg in messages:
            ...
            with ui.chat_message(
                ...
                "bg-color=grey-10 text-color=grey-1"
                if is_user
                else "bg-color=blue-grey-10 text-color=blue-grey-1"
            ):
                ui.markdown(
                    msg.content,
                    extras=[
...
async def main():
    ...
    with (
        ui.footer().props('class="dark:bg-stone-900"'),
        ui.column().classes("w-full max-w-3xl mx-auto my-6"),
    ):
        with ui.row().classes("w-full no-wrap items-center"):
            spinner = (
                ui.spinner(type="grid")
                .props('color="amber"')
                .style("visibility: hidden")
            )
            prompt = (
                ui.input(placeholder="prompt")
                .on("keydown.enter", lambda: send(prompt))
...
if __name__ in {"__main__", "__mp_main__"}:
    ...
    parser.add_argument(
        ...
        "--port",
        type=int,
        help="Explicit port to run the app on (disables auto selection)",
    )
