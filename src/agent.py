...
def create_agent(config: Config) -> AgentVAR:
    ...
    agent = AgentVAR(
        name="Agent VAR",
        model=create_model_from_llm_config(config.llm),
        system_prompt=config.agent.instructions,
        mcp_servers=create_mcp_clients(config.mcp),
    )
    return agent

async def run_agent(prompt: str, agent: AgentVAR) -> str:
